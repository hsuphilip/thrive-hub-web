...
export default function Calendar() {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: prog } = await supabase.from("programs")
      .select("id, estimated_minutes, sessions_per_week, exercises(id)")
      .eq("client_id", user.id).order("created_at", { ascending: false }).limit(1).single();

    const totalEx = prog?.exercises?.length ?? 0;
    const estMin = prog?.estimated_minutes ?? 30;
    const spw = prog?.sessions_per_week ?? 3;
    const progExIds = (prog?.exercises ?? []).map((e: any) => e.id);

    const q = supabase.from("exercise_feedback").select("exercise_id, created_at").eq("client_id", user.id);
    const { data: fb } = progExIds.length > 0 ? await q.in("exercise_id", progExIds) : await q;
    ...
    setDoneDays(done); setPartialDays(partial);

    const today = new Date();
    const monthDone = Array.from(done).filter((k) => { const [y,m] = k.split("-").map(Number); return y===year && m===month; }).length;
    setTotalSessions(monthDone);
    setTotalMinutes(monthDone * estMin);

    const daysIntoMonth = today.getMonth()===month && today.getFullYear()===year ? today.getDate() : new Date(year, month+1, 0).getDate();
    const target = Math.max(1, (daysIntoMonth/7) * spw);
    const active = Array.from(done).concat(Array.from(partial)).filter((k) => { const [y,m] = k.split("-").map(Number); return y===year && m===month; }).length;
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 py-6">
      ...
      {loading ? (
        <div className="flex justify-center mt-12"><div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" /></div>
      ) : (
        <>
          {/* Calendar */}
          ...
          <div className="bg-primary rounded-2xl p-6 mb-4">
            ...
            <p className="font-inter text-sm text-primary-container">{totalSessions === 1 ? "session completed" : "sessions completed"}</p>
          </div>
